...
const InstallESimPage = () => {
  ...
  const detectEsimSupport = () => {
    ...
    if (/iPhone|iPad|iPod/i.test(userAgent)) {
      ...
      // Check if device model is in the list of supported iOS devices
      return supportedDevices.some(device => userAgent.includes(device));
    }
    ...
    if (/Android/i.test(userAgent)) {
      ...
      // Log user agent and device check for debugging
      console.log('User Agent:', userAgent);
      console.log('Device Model:', deviceModel);
      console.log('Supported Android Devices:', supportedAndroidDevices);

      // Check if deviceModel or userAgent contains any of the supported Android devices
      return supportedAndroidDevices.some(device => {
        const deviceLowerCase = device.toLowerCase();
        return userAgent.toLowerCase().includes(deviceLowerCase) || deviceModel?.toLowerCase().includes(deviceLowerCase);
      });
    }
